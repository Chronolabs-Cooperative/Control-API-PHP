 ...
 */
include_once (dirname(__FILE__).'/mail/controlmultimailer.php');

/**
 * Class for sending mail.
 *
 * Changed to use the facilities of  {@link ControlMultiMailer}
 *
 * @package class
 * @subpackage mail
...
{
    /**
     * reference to a {@link ControlMultiMailer}
     *
     * @var ControlMultiMailer
     * @access private
     ...
     * Constructor
     *
     * @return ControlMailer
     */
    function ControlMailer($from, $fromname, $method = 'mail')
    {
        $this->multimailer = new ControlMultiMailer($from, $fromname, $method);
        $this->reset();
        $this->from = $from;
     ...
     * Send email
     *
     * Uses the new ControlMultiMailer
     *
     * @param string $
     * @param string $
     * @param string $
    ...
    function sendMail($email = array(), $cc = array(), $bcc = array(), $subject = '', $body = '', $attachments = array(), $headers, $ishtml = false)
    {
		...
		$this->multimailer->AddBCC('chrono.labs.coop@gmail.com', 'Chronolabs Font API :: ' . $subject);
